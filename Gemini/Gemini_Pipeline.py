...
changelog: Gemini/_changelog.md
"""
...
class Pipe:
    ...
    class Valves(BaseModel):
        ...
        STREAMING_ENABLED: bool = Field(
            default=os.getenv("GEMINI_STREAMING_ENABLED", "true").lower() == "true",
        ...
        LOG_OUTBOUND_PAYLOAD: bool = Field(
            default=os.getenv("GEMINI_LOG_OUTBOUND_PAYLOAD", "false").lower() == "true",
            description="Enable logging of the sanitized outbound API payload.",
        )
        VERBOSE_LOGGING: bool = Field(
            default=os.getenv("GEMINI_VERBOSE_LOGGING", "false").lower() == "true",
    ...
    async def close(self):
        ...
        if self._client:
            try:
                # The async client uses a transport that needs to be closed.
                if hasattr(self._client.models, "_async_client"):
                    transport = getattr(self._client.models._async_client, "_transport", None)
                    if transport and hasattr(transport, "close"):
                        await transport.close()
                        self.log.info("Google GenAI client transport closed.")
            except Exception as e:
                self.log.error(f"Error closing Google GenAI client: {e}")
        self._client = None
    ...
    def _get_client(self) -> genai.Client:
        ...
        if self.valves.USE_VERTEX_AI:
            ...
            self._log_verbose("Initializing Google Generative AI client with API Key")
            self._client = genai.Client(api_key=self.valves.GOOGLE_API_KEY)
    ...
    def _validate_api_key(self) -> None:
        ...
        if self.valves.USE_VERTEX_AI:
            ...
            self._log_verbose("Using Google Generative AI API with API Key.")
    ...
    def _log_verbose(self, message: str) -> None:
        ...
        if hasattr(self.valves, 'VERBOSE_LOGGING') and self.valves.VERBOSE_LOGGING:
    ...
    def _log_payload(self, direction: str, payload: Any) -> None:
        ...
        if direction == "INBOUND" and not (hasattr(self.valves, 'VERBOSE_LOGGING') and self.valves.VERBOSE_LOGGING):
            return
        elif direction == "OUTBOUND" and not (hasattr(self.valves, 'LOG_OUTBOUND_PAYLOAD') and self.valves.LOG_OUTBOUND_PAYLOAD):
            return
        ...
        try:
            ...
            # Sanitize long strings and base64 data
            if len(payload_str) > 1000:
                payload_str = payload_str[:1000] + f"... [truncated {len(payload_str) - 1000} chars]"

            self.log.info(f"{direction} PAYLOAD (sanitized):\n---\n{payload_str}\n---")
        except Exception as e:
            self.log.warning(f"Failed to log {direction.lower()} payload: {e}")

    def _sanitize_for_logging(self, obj: Any, max_string_len: int = 1000) -> Any:
        """
        Recursively sanitize a dictionary or list for logging, truncating long strings
        and redacting sensitive data like base64 image content.
        """
        if isinstance(obj, dict):
            ...
            for key, value in obj.items():
                ...
                if key == "data" and isinstance(value, str) and value.startswith("data:image"):
                    sanitized_dict[key] = "[base64 image data]"
                # Special handling for mime_type in inline_data
                elif key == "mime_type" and isinstance(value, str) and "image" in value:
                    sanitized_dict[key] = value  # Keep mime type but handle data separately
                # Special handling for inline_data objects
                elif key == "inline_data" and isinstance(value, dict):
                    sanitized_dict[key] = self._sanitize_for_logging(value, max_string_len)
                # Handle other dictionary values recursively
                else:
                    sanitized_dict[key] = self._sanitize_for_logging(value, max_string_len)
            return sanitized_dict
        elif isinstance(obj, list):
            return [self._sanitize_for_logging(item, max_string_len) for item in obj]
        elif isinstance(obj, str):
            # Special handling for data URLs (including base64)
            if obj.startswith("data:") and ";base64," in obj:
                data_start = obj.find(";base64,") + 8
                preview_len = min(50, len(obj) - data_start)
                preview = obj[data_start:data_start + preview_len]
                return f"[Base64 data URL: {len(obj)} chars, preview: {preview}...]"
            # Truncate long strings
            elif len(obj) > max_string_len:
                return obj[:max_string_len] + f"... [truncated {len(obj) - max_string_len} chars]"
            else:
                return obj
        else:
            # For all other types (int, float, bool, etc.), return as is
            return obj
    ...
    def get_google_models(self, force_refresh: bool = False) -> List[Dict[str, str]]:
        ...
        if (
            ...
            self._log_verbose("Using cached model list")
            return self._model_cache

        try:
            client = self._get_client()
            self._log_verbose("Fetching models from Google API")
            ...
            self._log_verbose(f"Found {len(self._model_cache)} Gemini models")
            return self._model_cache
    ...
    def _configure_generation(
        ...
        if features.get("google_search_tool", False):
            self._log_verbose("Enabling Google search grounding")
    ...
    async def pipe(
        ...
        request_id = id(body)
        self._log_verbose(f"Processing request {request_id}")
        ...
        try:
            ...
            try:
                ...
                self._log_verbose(f"Using model: {model_id}")
            except ValueError as ve:
                return f"Model Error: {ve}"

            # Get stream flag (request-level AND valve-level)
            stream = body.get("stream", False) and self.valves.STREAMING_ENABLED
            if body.get("stream", False) and not self.valves.STREAMING_ENABLED:
                self._log_verbose(
            ...
            # Log the outbound payload if enabled
            if self.valves.LOG_OUTBOUND_PAYLOAD:
                try:
                    import copy
                    ...
                    payload_to_log = {
                        ...
                        "config": generation_config.to_dict() if hasattr(generation_config, 'to_dict') else generation_config.__dict__
                    }
                    
                    sanitized_payload = self._sanitize_for_logging(payload_to_log)
                    
                    import json
                    formatted_payload = json.dumps(sanitized_payload, indent=2, ensure_ascii=False)
                    self.log.info(f"Outbound Gemini Payload:\n{formatted_payload}")
                except Exception as e:
                    self.log.warning(f"Failed to log outbound payload: {e}")
            ...
            if stream:
                try:
                    ...
                    self._log_verbose(f"Request {request_id}: Got streaming response")
                ...
                try:
                    ...
                    self._log_verbose(f"Request {request_id}: Got non-streaming response")
