...
class SystemeValeurs:
    ...
    def resoudre_dilemme(self, dilemme: Dict) -> str:
        ...
        for option in dilemme["options"]:
            ...
            score_contextuel = self.evaluer_contexte(dilemme["situation"], option)
            scores.append(score_ethique * 0.7 + score_contextuel * 0.3)  # Pondération
    ...
    def evaluer_contexte(self, situation: str, option: str) -> float:
        ...
        pertinence = len(mots_situation.intersection(mots_option)) / len(mots_situation)
        return pertinence
    ...
    def ajuster_emotions(self, evenement: str, intensite: float):
        ...
        for emotion in self.emotions:
            if emotion in evenement.lower():
                ...
                nouveau_niveau = max(0, min(1, self.emotions[emotion] + intensite))
        ...
        for option in options:
            ...
            for emotion, niveau in self.emotions.items():
                if emotion in option.lower():
                    score += niveau * self.impact_emotions[emotion].get("decision", 0.1)
            scores.append(score)
...
class MemoireAvancee:
    def __init__(self):
        ...
        self.memoire_sensorielle = SensorielleBuffer()
        ...
        self.index_semantique = IndexSemantique()

        # Gestionnaire de contexte
        self.contexte_manager = ContexteManager()

        # Métriques et statistiques
        self.metriques = MetriquesMemoire()
        ...
        for autre_id, autre_vecteur in self.index_semantique.items():
            if autre_id != nouveau_souvenir_id:
                ...
                if similarite > 0.5:  # Seuil de similarité
                    self.force_connexions[(nouveau_souvenir_id, autre_id)] = similarite
                    with self.conn:
                        self.conn.execute(
                            """
                            ...
                            VALUES (?, ?, ?, ?)
                        """,
                            (nouveau_souvenir_id, autre_id, similarite, "similarite"),
                        )
    ...
    def recuperer_episode(self, contexte: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        distances = [
            (
                episode,
                np.linalg.norm(
                    np.array(list(contexte.values())) - np.array(list(episode.values()))
                ),
            )
...
class ConscienceDeSoi:
    ...
    def _demarrer_pensee_background(self):
        ...
        thread_pensee = threading.Thread(target=penser_en_arriere_plan, daemon=True)
        thread_pensee.start()
    ...
    def evaluer_reponse(self, question: str, reponse: str, contexte: List[str]) -> Dict:
        ...
        if contexte:
            ...
            evaluation["utilisation_contexte"] = contexte_utilise / len(contexte)
        else:
...
class ModeleDeLangage:
    ...
    async def traiter_question(self, question: str) -> str:
        # Ajouter la question à la mémoire sensorielle
        self.memoire.memoire_sensorielle.ajouter_perception(
            {"type": "question", "contenu": question}
        )
        ...
        self.conscience.pensees_actives.put(
            f"Réflexion sur: {question} (Qualité: {evaluation['pertinence']:.2f})"
        )
