...
- This module uses two prompt files for its LLM operations:
  - Code/MIS_Identification_Prompt_v8.md: Used in Stage 1 for identifying potential memories from user messages
  - Prompts/MIS_Identification_Patterns.md: Regex and change indicator and attribute patterns for module valves parameters
  - Code/MIS_Integration_Prompt_v8.md: Used in Stage 2 for analyzing relevance between potential and existing memories (current version: v8)
...
class Filter:
    ...
    async def _integrate_potential_memories(
        ...
        for i, mem in enumerate(potential_memories, 1):
            ...
            # Format as a numbered list item with category and content
            if sub_memory:
                potential_mem_lines.append(f"{i}. {category}:\n- {sub_memory}")
            else:
                ...
                if ":" in content and not content.startswith(f"{category}:"):
                    ...
                    potential_mem_lines.append(f"{i}. {content}")
        ...
        for index, analysis in enumerate(llm_analysis_results):
            ...
            if original_intent == "delete":
                ...
                # Fallback: Find the memory to delete based on category and content match
                target_id = None
                ...
                for mem in db_memories:
                    ...
                    if mem_category.lower() == category.lower():
                        # Check if any item in this memory matches the sub_memory to delete
                ...
                if target_id and target_desc:
                    ...
                    logger.info(
                        f"Could not find matching memory for delete intent: category '{category}', content '{sub_memory}'"
                    )
                continue  # Skip further processing for this item
            ...
            if original_intent == "negate":
                ...
                for comp in comparisons:
                    ...
                    if relevance >= self.valves.memory_relevance_threshold and mem_id and relevance > max_relevance:
                ...
                if high_relevance_memory_id:
                    ...
                    for mem in db_memories:
                        ...
                        for item in mem_items:
                            ...
                            keywords = [word.lower() for word in sub_memory.split() if len(word) > 3]
        ...
        for action_data in operations:
            ...
            try:
                if action_type == "CREATE":
                    ...
                    for bullet in bullets_to_add:
                        ...
                        if self.valves.replacement_patterns_enabled:
                            ...
                            for pattern in self.valves.change_indicators:
                                indicator_match = re.search(pattern, clean_bullet, re.IGNORECASE)
                                if indicator_match:
                                    change_indicator_found = True
                                    matching_indicator = pattern
                                    logger.info(f"Change indicator found in bullet: '{clean_bullet}' (matched pattern: '{pattern}')")
                                    break
                            ...
                            if change_indicator_found:
                                ...
                                for i, pattern in enumerate(self.valves.attribute_patterns):
                                    if re.search(pattern, clean_bullet, re.IGNORECASE):
                                        matching_pattern_index = i
                                        matching_pattern = pattern
                                        logger.info(f"Attribute pattern matched: '{pattern}'")
                                        break
                                
                                # If we found a matching pattern
                                if matching_pattern:
                                    # Extract the attribute key from the new bullet
                                    new_match = re.search(matching_pattern, clean_bullet, re.IGNORECASE)
                                    if new_match and new_match.groups():
                                        # The first capture group is usually the attribute key
                                        new_attribute_key = new_match.group(1).lower() if new_match.group(1) else None
                                        logger.info(f"Extracted attribute key from new bullet: '{new_attribute_key}'")
                                        
                                        # Only look for existing items that match the SAME pattern
                                        for existing_item in current_items:
                                            existing_match = re.search(matching_pattern, existing_item, re.IGNORECASE)
                                            if existing_match and existing_match.groups():
                                                # Extract the attribute key from the existing item
                                                existing_attribute_key = existing_match.group(1).lower() if existing_match.group(1) else None
                                                logger.info(f"Checking existing item: '{existing_item}' with attribute key: '{existing_attribute_key}'")
                                                
                                                # Only replace if they're about the same attribute
                                                if new_attribute_key and existing_attribute_key and new_attribute_key == existing_attribute_key:
                    ...
                    # Find the item to remove (exact match)
                    item_found = False
                    updated_items = []
                    for item in current_items:
                        if clean_item_desc.lower() == item.lower():
                            item_found = True
    ...
    async def _send_citation(
        ...
        for category_header, data in grouped_results.items():
            ...
            action_description = ""
            has_adds = any(item.get("action") in ["CREATE", "MERGE_UPDATE"] and item.get("success") for item in items_details)
            has_removes = any(item.get("action") == "REMOVE_ITEM" and item.get("success") for item in items_details)

            # Prioritize description based on actions
            if has_adds and not has_removes:
                 # Check if it was a CREATE or MERGE_UPDATE
                 create_op = next((item for item in items_details if item.get("action") == "CREATE"), None)
                 if create_op:
                 ...
                 delete_op = next((item for item in items_details if item.get("action") == "REMOVE_ITEM"), None)
                 if delete_op and delete_op.get("status") == "Deleted (Last Item Removed)":
                      # If the original category was Miscellaneous or Unknown, use "No category"
                      # We need to parse the category_header to check this
                      category_name = category_header.rstrip(':') if isinstance(category_header, str) else "Unknown"
            ...
            clean_category = category_header.rstrip(':') if isinstance(category_header, str) else "Unknown"

            # Handle "No category" specifically for deletions
            if action_description == "(Deleted info)" and (clean_category == "Miscellaneous" or clean_category == "Unknown"):
            ...
            if not final_content and not any(item.get("action") == "REMOVE_ITEM" and item.get("success") for item in items_details):
                continue
        ...
        await self._safe_emit(
            emitter,
            {
                ...
                "data": {
                    ...
                    "source": {"name": "Memories Identified"},
                },
            },
        )
