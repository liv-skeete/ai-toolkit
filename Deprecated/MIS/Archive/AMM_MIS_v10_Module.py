...
class Filter:
            ...
            if hasattr(self.valves, key):
                ...
                if key == "identification_prompt":
                    if isinstance(value, str) and value.strip():
                        preview = value[:50].replace('\n', '\\n') + "..." if len(value) > 50 else value.replace('\n', '\\n')
                        logger.info(f"Updating {key} with preview: {preview}")
                        setattr(self.valves, key, value)
                    else:
                        logger.warning(f"Received empty or non-string value for identification_prompt: type={type(value)}")
                        # Ensure it's at least an empty string if invalid value received
                        setattr(self.valves, key, "")
                # Handle other valve types
                elif key != "identification_prompt": # Avoid logging prompt again if handled above
    ...
    async def _identify_potential_memories(
        ...
        if self.valves.verbose_logging:
            ...
            logger.info("System prompt for Identification (truncated):\n%s", truncated_prompt)
    ...
    async def _identify_and_prepare_memories(
        ...
        important_memories = [
            mem for mem in potential_memories
            if mem.get("importance", 0) >= self.valves.memory_importance_threshold
        ]

        if not important_memories:
            logger.info("No potential memories met the importance threshold (%.2f). No operations needed.", self.valves.memory_importance_threshold)
            return []
        ...
        memory_operations = [
            {"operation": "NEW", "content": mem["content"], "importance": mem["importance"]}
    ...
    async def _query_api(self, provider: str, messages: List[Dict[str, Any]]) -> str:
        ...
        while retries <= self.valves.max_retries:
            ...
            except (aiohttp.ClientError, asyncio.TimeoutError, ValueError, json.JSONDecodeError, KeyError) as e:
                logger.warning(f"API call failed (attempt {retries + 1}/{self.valves.max_retries + 1}): {e}")
                retries += 1
                if retries > self.valves.max_retries:
                    logger.error("Max retries exceeded. Failing operation.")
                    raise  # Re-raise the last exception
                await asyncio.sleep(self.valves.retry_delay * (2 ** (retries - 1))) # Exponential backoff
        ...
        try:
            ...
            if match:
                ...
                if not json_str.startswith(('[', '{')):
                    json_str = json_str[json_str.find('['):] if '[' in json_str else json_str[json_str.find('{'):]
                if not json_str.endswith((']', '}')):
                     json_str = json_str[:json_str.rfind(']')+1] if ']' in json_str else json_str[:json_str.rfind('}')+1]
            ...
            if isinstance(parsed_data, dict):
                ...
                if all(isinstance(item, dict) for item in parsed_data):
                    ...
                    logger.warning("API returned a list containing non-dictionary items. Discarding.")
                    return []
            else:
                logger.warning(f"Parsed JSON is not a dictionary or list, but {type(parsed_data)}. Discarding.")
                return []

        except json.JSONDecodeError as e:
            logger.error(f"Failed to decode JSON response: {e}")
            if self.valves.verbose_logging:
                 logger.error("Raw response causing error:\n%s", self._truncate_log_lines(response_text))
            return []
        except Exception as e:
    ...
    async def _execute_memory_creation(
        self, memory_operations: List[Dict[str, Any]], __user__: dict, __event_emitter__: Optional[Callable[..., Awaitable[None]]] = None
    ) -> None:
        """
        ...
        """
        logger.info("Executing %d NEW memory creation operations", len(memory_operations))
        ...
        for op in memory_operations:
            ...
            if operation_type == "NEW":
                ...
                if content:
                    logger.info("Attempting to CREATE memory for user %s (Importance: %.2f): %s", __user__.get('id', 'UNKNOWN'), importance, content[:100] + "...")
                    ...
                    status_updates_with_importance.append(("⚠️ Skipped creating memory: missing content", 0))
            else:
                # This case should not happen after refactoring
                logger.warning("Skipping unexpected operation type '%s': %s", operation_type, op)
                # Unknown operations get importance 0 for sorting
                status_updates_with_importance.append((f"⚠️ Skipped unknown operation: {operation_type}", 0))

        # Sort status updates by importance (highest first)
        sorted_status_updates = [status for status, _ in sorted(status_updates_with_importance,
        ...
        if self.valves.show_status and __event_emitter__ and sorted_status_updates:
             ...
             await self._send_citation(__event_emitter__, sorted_status_updates, __user__.get('id')) # Fix NameError
    ...
    async def _safe_emit(
        self,
        emitter: Callable[[Dict[str, Any]], Awaitable[None]], # Emitter expects one Dict arg
        data: Dict[str, Any] # Data dict includes the 'type' (e.g., "status", "citation")
    ) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to emit event type '{event_type}': {e}", exc_info=True)
    ...
    def _non_blocking_emit(
        ...
        if emitter:
            asyncio.create_task(self._safe_emit(emitter, data)) # Pass only emitter and data
        else:
            event_type = data.get("type", "UNKNOWN")
            logger.debug(f"Emitter not available, cannot emit event type '{event_type}'")
    ...
    async def _send_citation(
        ...
        logger.info("Sending citation for user %s:\n%s", user_id or 'UNKNOWN', citation_message)
        ...
        await self._safe_emit(
            emitter,
            {
                ...
                "data": {
                    ...
                    "metadata": [{"source": "module://mis/memories", "html": False}],  # Standard metadata format
                    "source": {"name": "Memories Saved"},  # Title for the citation (matching backup file)
                },
            }
        )
  ...
  # --------------------------------------------------------------------------
    ...
    async def inlet(
        ...
        try:
            ...
            logger.info(
                "Memory processing finished in %.2f seconds", duration
            )
    ...
    async def outlet(self, body: Dict[str, Any], __user__: dict = {}) -> Dict[str, Any]: # Changed parameter name and added default
        """
