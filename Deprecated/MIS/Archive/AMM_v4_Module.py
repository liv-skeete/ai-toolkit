"""
title: Automatic Memory Manager for Open WebUI v4
...
date: 2025-03-06
changes:
- standardized error handling across all API methods
- added helper method for consistent memory ID formatting
- fixed inconsistent filtering in _should_process_conversation
...
class Filter:
    ...
    class Valves(BaseModel):
        ...
        # Role-aware Conversation processing
        role_aware_processing: bool = Field(
            default=True,  # Start with it enabled
            description="Enable role-aware conversation processing",
        )
...
**Handling Assistant Summaries:**
- The assistant may summarize existing memories in its messages.
- Do NOT create new memories based solely on the assistant's summaries.
- If the assistant's summary contains new information about the user, consider updating the existing memory with the new information.
...
"""
    ...
    def _format_existing_memories(self, db_memories: List[Any]) -> List[str]:
        return [
            self._format_memory_with_id(mem.id, mem.content)
    ...
    async def process_conversation(
        self, conversation_history: List[Dict[str, Any]], user_id: str, user: Any
    ) -> Dict[str, Any]:
        """
        ...
        filtered_history = [
            ...
            and not msg.get("memory_system_message", False)
        ]
        ...
        if db_memories:
            ...
            memories_context = "Existing memories:\n" + "\n".join(
                self._format_memory_with_id(mem.id, mem.content, include_bullet=True)
                for mem in db_memories
                if hasattr(mem, "content") and hasattr(mem, "id")
            )
            ...
            try:
                ...
                if memory_operations and isinstance(memory_operations, list):
                    ...
                    if valid_operations:
                        ...
                        return {
                            "processed": False,
                            "reason": "No valid memory operations found in response",
                        }
                else:
                    return {
                        "processed": False,
                        "reason": "Response did not contain a valid list of operations",
                    }
            except Exception as e:
                return {
                    "processed": False,
                    "reason": f"Error processing conversation: {str(e)}",
                }
        except Exception as e:
    ...
    def _should_process_conversation(self, messages: List[Dict[str, Any]]) -> bool:
        ...
        user_messages = [
            m for m in messages
            if m["role"] == "user"
            and not m.get("is_memory_summary", False)
            and not m.get("memory_system_message", False)
        ]
    ...
    async def outlet(
        ...
        if "messages" in body and __user__:
            try:
                ...
                if (
                    self.valves.role_aware_processing
                    and self._should_process_conversation(body["messages"])
                ):
                    user = Users.get_user_by_id(__user__["id"])
                    if user:
                        # Process the conversation
                        result = await self.process_conversation(
                            body["messages"], __user__["id"], user
                        )
    ...
    async def identify_memories(
        ...
        if (
            not self.valves.api_provider == "OpenAI API"
            and not self.valves.openai_api_key
        ):
    ...
    async def get_relevant_memories(
        ...
        if not (
            (self.valves.api_provider == "OpenAI API" and self.valves.openai_api_key)
            or (self.valves.api_provider == "Ollama API")
        ):
            return []
        try:
            if db_memories is None:
...
"""
            ...
            try:
                ...
                try:
                    ...
                    safe_cleaned_response = (
                        cleaned_response.replace("%", "%%") if cleaned_response else ""
                    )
                    cleaned_response = re.sub(
                        r"[^\x20-\x7E]", "", safe_cleaned_response
                    )
                    try:
                        relevant_memories = json.loads(cleaned_response)
                        if not isinstance(relevant_memories, list):
                ...
                for mem_obj in relevant_memories:
                    ...
                    if isinstance(mem_obj, dict) and "text" in mem_obj:
                        ...
                        if (
                            self.valves.relevance_threshold > 0
                            and mem_score < self.valves.relevance_threshold
                        ):
                            continue
