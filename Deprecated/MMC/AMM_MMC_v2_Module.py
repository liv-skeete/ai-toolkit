...
class Filter:
    ...
    class Valves(BaseModel):
        ...
        # Consolidation settings (Note: Similarity is now guided by prompt examples/rules, not a numerical threshold)
    ...
    def _parse_memory_content(self, content: str) -> Tuple[str, List[str]]:
        ...
        for line in lines[1:]:
            ...
            elif stripped_line: # Handle potential non-bullet lines if needed (currently ignored)
                 logger.debug(f"Ignoring non-bullet line in memory content: '{stripped_line}'")
    ...
    async def _process_memory_operations(
        ...
        for memory_id, items_to_delete in delete_operations_by_memory.items():
            ...
            # Find items to remove (case-insensitive matching)
            items_to_keep = []
            removed_items = []
            ...
            for existing_item in existing_items:
                ...
                for delete_item in items_to_delete:
                    ...
                    if delete_item.lower() == existing_item.lower():
                        should_keep = False
    ...
    async def _process_consolidation_background(
        ...
        try:
            ...
            if processed_operations and event_emitter:
                ...
                citation_content = "Memory Consolidation Results:\n\n"
                
                for category, ops in operations_by_category.items():
                    citation_content += f"## {category}\n"
                    
                    for op in ops:
                        operation = op.get("operation", "UNKNOWN")
                        
                        if operation == "NEW":
                            citation_content += "✅ Created new memory block\n"
                            # Show the items that were added
                            for item in op.get("affected_items", []):
                                citation_content += f"  • {item}\n"
                                
                        elif operation == "UPDATE":
                            # Check if this was an add or remove operation
                            if "Added" in op.get("status", ""):
                                citation_content += "✅ Added to existing memory:\n"
                                for item in op.get("affected_items", []):
                                    citation_content += f"  • {item}\n"
                            elif "Removed" in op.get("status", ""):
                                citation_content += "❎ Removed from memory:\n"
                                for item in op.get("affected_items", []):
                                    citation_content += f"  • {item}\n"
                            else:
                                citation_content += f"✅ Updated memory: {op.get('status', '')}\n"
                                
                        elif operation == "DELETE":
                            citation_content += "❎ Deleted memory block\n"
            ...
            if event_emitter:
                ...
                # Schedule status clear *after* sending the final message
                if event_emitter:
                    clear_task = asyncio.create_task(self._delayed_clear_status(event_emitter))
                    self.background_tasks.add(clear_task)
                    clear_task.add_done_callback(self.background_tasks.discard)
